...
7. [KẾT QUẢ ĐẠT ĐƯỢC & HƯỚNG PHÁT TRIỂN](#7-kết-quả-đạt-được--hướng-phát-triển)
   - 7.1. Tổng kết đánh giá kết quả đạt được
   - 7.2. Thực nghiệm Demo chi tiết từng chức năng hệ thống (Kèm hình ảnh minh chứng)
     - 7.2.1. Phân hệ Đăng nhập & Xác thực bảo mật BCrypt
     - 7.2.2. Phân hệ Nhật ký Hệ thống (Audit Log) - Vai trò Quản trị viên (Admin)
     - 7.2.3. Phân hệ Tổng quan Dashboard Quản lý (KPIs & Biểu đồ nâng cao)
     - 7.2.4. Phân hệ Sơ đồ Bàn ăn Thời gian thực (Table Map)
     - 7.2.5. Phân hệ Gọi món tại bàn & Chuyển bàn (Order Management)
     - 7.2.6. Phân hệ Điều hành Bếp & Quầy Bar (Kitchen & Bar Operations)
     - 7.2.7. Phân hệ Thu ngân, Áp dụng Voucher & Thuế VAT (Checkout & Payment)
     - 7.2.8. Phân hệ Quản lý Ca làm việc & Đối soát Két tiền mặt (Shift Management)
     - 7.2.9. Phân hệ Lịch sử Hóa đơn & Tra cứu Thanh toán (Invoice History)
     - 7.2.10. Các Phân hệ Quản trị Dữ liệu Danh mục (Master Data CRUD)
     - 7.2.11. Báo cáo Thống kê Doanh thu & Top Món ăn Bán chạy
   - 7.3. Kế hoạch và Kết quả Kiểm thử Hệ thống (System Testing & Test Cases)
     - 7.3.1. Mục tiêu và Môi trường Kiểm thử (Testing Environment)
     - 7.3.2. Bảng Danh mục Chi tiết các Kịch bản Kiểm thử (Test Cases Specification)
     - 7.3.3. Đặc tả Kỹ thuật Chi tiết các Ca Kiểm thử Trọng yếu
     - 7.3.4. Nhật ký Thực thi Kiểm thử Tự động (Automated Test Execution Log - JUnit 5)
     - 7.3.5. Đánh giá Mức độ Tin cậy và Độ bao phủ (Reliability & Coverage Assessment)
...
Quy trình phát triển đề tài được hoạch định theo mô hình lặp (Iterative Waterfall) trong vòng **3 tuần**:
...
Hệ thống phân định rạch ròi **5 vai trò tác nhân** với quyền hạn được kiểm soát chặt chẽ từ khâu Đăng nhập (RBAC - Role-Based Access Control):
...
```
         ...
         ┌───────────────────┬─────────────────────┼─────────────────────┬───────────────────┐
         ▼                   ▼                     ▼                     ▼                   ▼
   [ QUẢN TRỊ VIÊN ]    [ QUẢN LÝ ]           [ THU NGÂN ]          [ PHỤC VỤ ]        [ BẾP TRƯỞNG ]
     (Role: Admin)     (Role: QuanLy)       (Role: ThuNgan)        (Role: PhucVu)        (Role: Bep)
         │                   │                     │                     │                   │
 ┌───────┴───────┐   ┌───────┴───────┐     ┌───────┴───────┐     ┌───────┴───────┐   ┌───────┴───────┐
 │ • Xem AuditLog│   │ • TOÀN QUYỀN  │     │ • Thu ngân    │     │ • Sơ đồ bàn   │   │ • Điều hành   │
 │ • Dashboard   │   │ • Dashboard   │     │ • Sơ đồ bàn   │     │ • Gọi món     │   │ • Nhận nấu    │
 │ • Quản trị DB │   │ • Báo cáo DT  │     │ • Mở/Đóng ca  │     │ • Chuyển bàn  │   │ • Báo ra món  │
 │ (ẨN Bán hàng/ │   │ • Quản trị DB │     │ • Xem HĐ cũ   │     └───────────────┘   │ • Báo hết món │
 │  Thu ngân/DT) │   │ • Bàn/Bếp/HĐ  │     └───────────────┘                         └───────────────┘
 └───────────────┘   └───────────────┘
```

#### Ma trận quyền hạn chi tiết giữa các vai trò

| Nhóm chức năng / Module | Admin (Quản trị viên) | Quản lý (QuanLy) | Thu ngân (ThuNgan) | Phục vụ (PhucVu) | Bếp trưởng (Bep) |
|---|:---:|:---:|:---:|:---:|:---:|
| **Nhật ký hệ thống (Audit Log)** | ✅ **Có** (Ưu tiên) | ✅ Có | ❌ Ẩn | ❌ Ẩn | ❌ Ẩn |
| **Tổng quan Dashboard** | ✅ Có | ✅ **Có** (Ưu tiên) | ❌ Ẩn | ❌ Ẩn | ❌ Ẩn |
| **Quản trị Dữ liệu (User, Menu, Bàn, Voucher)** | ✅ Có | ✅ Có | ❌ Ẩn | ❌ Ẩn | ❌ Ẩn |
| **Sơ đồ bàn ăn & Gọi món (Order)** | ❌ Ẩn | ✅ Có | ✅ Có | ✅ Có | ❌ Ẩn |
| **Quản lý Bếp & Bar chế biến** | ❌ Ẩn | ✅ Có | ❌ Ẩn | ❌ Ẩn | ✅ Có |
| **Thu ngân (Checkout) & Ca làm việc** | ❌ Ẩn | ✅ Có | ✅ Có | ❌ Ẩn | ❌ Ẩn |
| **Báo cáo Doanh thu & Top món chạy** | ❌ Ẩn | ✅ Có | ❌ Ẩn | ❌ Ẩn | ❌ Ẩn |

#### Danh sách tài khoản mặc định kiểm thử hệ thống

| Tên đăng nhập | Mật khẩu | Vai trò | Chức năng phụ trách |
|:---|:---|:---:|:---|
| `admin` | `admin123` | **Admin** | Quản trị viên hệ thống (Xem Audit Log & Quản trị dữ liệu) |
| `admin02` | `admin123` | **Admin** | Quản trị viên hệ thống mới (Nguyễn Phương Thảo) |
| `quanly` | `quanly123` | **QuanLy** | Quản lý điều hành toàn diện nhà hàng (**Toàn quyền**) |
| `cashier01` | `mai123` | **ThuNgan** | Thu ngân ca trực (Checkout, Ca làm việc, Lịch sử hóa đơn) |
| `waiter01` | `nam123` | **PhucVu** | Nhân viên phục vụ bàn (Sơ đồ bàn, Gọi món tại bàn) |
| `chef01` | `tuan123` | **Bep** | Bếp trưởng điều phối chế biến món ăn & Báo hết hàng |

> **Ghi chú bảo mật**: Toàn bộ mật khẩu trong hệ thống được mã hóa một chiều bằng thuật toán **BCrypt** (`$2a$10$...`) lưu trữ an toàn trong bảng `NhanVien`.
...
#### 1. Chức năng Đăng nhập & Xác thực Phân quyền (Authentication & Audit)
- **Mục đích**: Bảo vệ an toàn dữ liệu hệ thống, ngăn chặn truy cập trái phép, định danh nhân viên phục vụ/thu ngân chịu trách nhiệm trên từng đơn hàng, tự động ghi nhận vào Nhật ký hệ thống (Audit Log).
- **Ai sử dụng**: Toàn bộ nhân sự nhà hàng (Admin, Quản lý, Thu ngân, Phục vụ, Bếp trưởng).
- **Cách thức thực hiện**:
  1. Người dùng mở ứng dụng, màn hình `LoginDialog` hiển thị yêu cầu thông tin.
  2. Nhập `Tên đăng nhập` (ví dụ: `admin`, `admin02`, `quanly`, `cashier01`, `waiter01`, `chef01`).
  3. Nhập `Mật khẩu`.
  4. Nhấn nút `ĐĂNG NHẬP` hoặc nhấn phím `Enter`.
  5. Hệ thống kiểm tra trong bảng `NhanVien` bằng thuật toán đối soát BCrypt:
     - Nếu thành công: Ghi nhận sự kiện `DangNhap` vào `LichSuTruyCap` kèm IP, khởi tạo `MainFrame` với thanh Sidebar được cấu hình phân quyền tương ứng:
       - Tài khoản **Admin**: Mặc định mở ngay màn hình `Nhật ký hệ thống (Audit Log)`.
       - Tài khoản **Quản lý**: Mặc định mở ngay màn hình `Tổng quan Dashboard`.
       - Tài khoản **Phục vụ / Thu ngân**: Mặc định mở `Sơ đồ bàn ăn`.
       - Tài khoản **Bếp**: Mặc định mở `Quản lý Bếp & Bar`.
     - Nếu thất bại: Ghi nhận sự kiện `DangNhapThatBai` vào `LichSuTruyCap` và báo lỗi trên giao diện.
- **Dữ liệu vào / ra**:
  - *Đầu vào*: Username (chuỗi), Password (chuỗi ký tự ẩn).
  - *Đầu ra*: Đối tượng `NhanVien` kèm vai trò (`VaiTro`), trạng thái truy cập ứng dụng.
- **Ràng buộc nghiệp vụ**: Tài khoản phải ở trạng thái `DangLam`. Nếu sai mật khẩu sẽ từ chối truy cập và ghi vết an ninh.
...
# 7. KẾT QUẢ ĐẠT ĐƯỢC & HƯỚNG PHÁT TRIỂN
...
2. **Đáp ứng toàn diện các tiêu chí học thuật và kỹ thuật của học phần**:
   ...
   - **Tối thiểu 5 giao diện/màn hình**: Đã xây dựng và đặc tả chi tiết **10 màn hình nghiệp vụ chính** với Look & Feel FlatLaf hiện đại, trực quan, chuyên nghiệp.
   - **Cơ chế xử lý Đa luồng Bất đồng bộ (SwingWorker)**: Sử dụng `AuditSwingWorker` và SwingWorker trong các tác vụ tải dữ liệu nặng từ CSDL nhằm đảm bảo giao diện luôn mượt mà, không bị treo (non-blocking EDT).
   - **An toàn & Bảo mật thông tin**: Mã hóa mật khẩu nhân viên bằng thuật toán **BCrypt** an toàn, kiểm soát phân quyền RBAC 5 vai trò rành mạch, tự động lưu vết nhật ký an ninh hệ thống (Audit Log).
3. **Độ tin cậy và chất lượng phần mềm**:
   - Toàn bộ **11/11 kịch bản kiểm thử tự động (JUnit 5)** đều vượt qua (`BUILD SUCCESS`).
...
Dưới đây là phần trình bày kết quả chạy thực nghiệm thực tế của toàn bộ các phân hệ chức năng trên môi trường Desktop Java Swing kết nối CSDL MySQL:

#### 7.2.1. Phân hệ Đăng nhập & Xác thực bảo mật BCrypt (Login & Security)
- **Mô tả kịch bản demo**:
  1. Khởi chạy ứng dụng, hộp thoại `LoginDialog` xuất hiện trang trọng ở giữa màn hình.
  2. Đèn trạng thái hiển thị: `● MySQL Online` màu xanh lá xác nhận kết nối CSDL MySQL 8.0 thành công.
  3. Người dùng nhập Tên đăng nhập và Mật khẩu (ví dụ: `admin`, `quanly`, `cashier01`, `waiter01`, `chef01`).
  4. Hệ thống kiểm tra mật khẩu đã được băm bằng thuật toán **BCrypt** an toàn. Nếu thành công, hệ thống tự động ghi nhật ký vào bảng `LichSuTruyCap` và chuyển tiếp đến màn hình chính theo đúng quyền hạn vai trò.
- **Hình ảnh kết quả demo**:

![Màn hình Đăng nhập & Xác thực bảo mật BCrypt](screenshots/ui_login.png)

---

#### 7.2.2. Phân hệ Nhật ký Hệ thống (Audit Log) - Dành riêng cho Quản trị viên (Admin)
- **Mô tả kịch bản demo**:
  1. Đăng nhập bằng tài khoản Quản trị viên (`admin` hoặc `admin02`).
  2. Hệ thống áp dụng ma trận RBAC: Tự động ẩn toàn bộ các chức năng vận hành kinh doanh (Bán hàng, Thu ngân, Bếp, Báo cáo doanh thu), chỉ hiển thị nhóm **HỆ THỐNG** và **QUẢN TRỊ DỮ LIỆU**.
  3. Màn hình khởi đầu mặc định mở ngay **Nhật ký hệ thống (Audit Log)**:
     - **Tab 1 - Lịch sử truy cập**: Hiển thị bảng nhật ký đăng nhập, đăng xuất, đăng nhập thất bại với thông tin thời gian, tên đăng nhập, họ tên, địa chỉ IP, trạng thái thành công/thất bại và ghi chú lý do.
     - **Tab 2 - Lịch sử thao tác**: Hiển thị toàn bộ các thao tác chỉnh sửa dữ liệu (`INSERT`, `UPDATE`, `DELETE`), bảng dữ liệu bị tác động, nội dung bản ghi cũ/mới, tên máy tính và địa chỉ IP thực hiện.
     - Bộ lọc ngày tháng (Từ ngày - Đến ngày), lọc loại hành động và nút **Xuất Excel**.
- **Hình ảnh kết quả demo**:

![Giao diện Phân hệ Nhật ký Hệ thống (Audit Log) - Vai trò Admin](screenshots/ui_audit_log.png)

---

#### 7.2.3. Phân hệ Tổng quan Dashboard Quản lý (Executive Dashboard)
- **Mô tả kịch bản demo**:
  1. Đăng nhập bằng tài khoản Quản lý (`quanly` - Toàn quyền cả hệ thống).
  2. Màn hình khởi đầu mặc định mở **Tổng quan Dashboard** và thanh Sidebar hiển thị đầy đủ 100% tất cả các nhóm chức năng.
  3. Màn hình Dashboard truy vấn dữ liệu từ MySQL và hiển thị đồ họa phân tích:
     - **4 Thẻ KPI**: Tổng doanh thu (23.8M VND), Tổng đơn hàng (30 đơn), Tỷ lệ lấp đầy bàn (30.0%), Thời gian phục vụ trung bình.
     - **Biểu đồ Phễu Vận hành (Funnel Chart)**: Thể hiện số lượng bàn và tiến trình phục vụ phân tầng (Tổng số bàn $\to$ Bàn có khách $\to$ Đã gọi món $\to$ Đang chế biến $\to$ Đã ra món).
     - **Biểu đồ Doanh thu & Mục tiêu (Bar & Spline Chart)**: Kết hợp cột Gradient hiển thị doanh thu thực tế và đường cong Bezier Spline mượt mà thể hiện mục tiêu kinh doanh.
     - **Bảng Top Món Ăn Bán Chạy**: Liệt kê số lượng bán và doanh thu đóng góp của từng món ăn hàng đầu.
- **Hình ảnh kết quả demo**:

![Giao diện Tổng quan Dashboard Quản lý Vận hành](screenshots/ui_manager_dashboard.png)

---

#### 7.2.4. Phân hệ Sơ đồ Bàn ăn Thời gian thực (Table Map)
- **Mô tả kịch bản demo**:
  1. Điều hướng tới mục **Sơ đồ bàn ăn**.
  2. Màn hình hiển thị mạng lưới 20 bàn ăn phân bố trực quan theo 3 khu vực: *Tầng 1*, *Tầng 2*, *Phòng VIP*.
  3. Trạng thái bàn được trực quan hóa bằng mã màu chuẩn công nghiệp:
     - **Màu Xanh lá**: Bàn trống, sẵn sàng đón khách mới.
     - **Màu Đỏ tươi**: Bàn đang có khách, hiển thị số lượng khách và tên nhân viên phục vụ phụ trách.
  4. Người dùng nhấp vào bàn Xanh để xác nhận mở bàn mới; nhấp vào bàn Đỏ để truy cập nhanh Order hoặc thanh toán Checkout.
- **Hình ảnh kết quả demo**:

![Giao diện Sơ đồ Bàn ăn Trực quan Thời gian thực](screenshots/ui_table_map.png)

---

#### 7.2.5. Phân hệ Gọi món tại bàn & Chuyển bàn (Order Management)
- **Mô tả kịch bản demo**:
  1. Tại màn hình Order, thông tin Bàn đang phục vụ hiển thị rõ ràng ở tiêu đề.
  2. **Vùng bên trái**: Bảng danh sách món ăn đã gọi với số lượng, đơn giá, thành tiền, cột trạng thái bếp (`Chờ nấu`, `Đang nấu`, `Đã ra món`), và ô tổng tiền tạm tính.
  3. **Vùng bên phải**: Danh mục thực đơn phân loại theo Tabs (Khai vị, Món chính, Hải sản, Đồ uống, Tráng miệng) kèm hình ảnh minh họa sinh động. Ô tìm kiếm hỗ trợ gõ tiếng Việt không dấu tự động lọc món ăn tức thì.
  4. Nhân viên có thể tăng/giảm số lượng món, nhập ghi chú chế biến riêng cho từng món, nhấn nút **Chuyển bàn** để đổi vị trí cho khách, và bấm **GỬI BẾP CHẾ BIẾN** để chuyển order xuống nhà bếp.
- **Hình ảnh kết quả demo**:

![Giao diện Màn hình Gọi món tại bàn (Order)](screenshots/ui_order_details.png)

---

#### 7.2.6. Phân hệ Điều hành Bếp & Quầy Bar (Kitchen & Bar Operations)
- **Mô tả kịch bản demo**:
  1. Màn hình dành riêng cho Bếp trưởng và nhân viên pha chế quầy bar.
  2. **Tab 1 - Hàng chờ chế biến**:
     - 4 thẻ thống kê: *Món chờ nấu*, *Đang chế biến*, *Cảnh báo chờ lâu (>15 phút)* chuyển màu đỏ nổi bật, và *Đã hoàn thành*.
     - Bảng danh sách món chờ theo thứ tự gọi món, hỗ trợ **chọn nhiều dòng cùng lúc (Multi-select)** bằng giữ phím `Ctrl` hoặc `Shift`.
     - Bếp trưởng bấm nút **🍳 Bắt đầu nấu** hoặc **✅ Hoàn thành (Ra món)** cho toàn bộ các món đã chọn chỉ trong 1 thao tác.
     - Có thể bật chế độ tự động làm mới sau mỗi 10 giây qua Swing Timer.
  3. **Tab 2 - Kho thực đơn & Báo hết món**: Cho phép bếp bấm **🔴 Báo Hết Hàng** (món ăn lập tức bị gạch ngang và chặn order trên màn hình Phục vụ) hoặc **🟢 Mở Bán Lại** khi đã chuẩn bị xong nguyên liệu.
- **Hình ảnh kết quả demo**:

![Giao diện Phân hệ Điều hành Bếp & Quầy Bar](screenshots/ui_kitchen_management.png)

---

#### 7.2.7. Phân hệ Thu ngân, Áp dụng Voucher & Thuế VAT (Checkout & Payment)
- **Mô tả kịch bản demo**:
  1. Thu ngân chọn bàn có khách cần thanh toán từ danh sách bên trái.
  2. Bảng chi tiết hóa đơn tự động tính toán tổng tiền tạm tính từ các món đã gọi.
  3. **Khung thanh toán thông minh bên phải**:
     - Hỗ trợ nhập mã khuyến mãi Voucher (ví dụ: `LEHOI10`, `VIP20`, `GIAM50K`): Hệ thống tự động xác thực hạn dùng, giá trị đơn tối thiểu và tính số tiền chiết khấu tối đa.
     - Tùy chọn thuế suất VAT linh hoạt (0%, 8%, 10%) bằng Radio Buttons và tự động tính tiền thuế.
     - Lựa chọn phương thức thanh toán: *Tiền mặt*, *Chuyển khoản (QR Code)*, *Thẻ tín dụng/ATM*.
     - Nút **Tách Hóa Đơn (Split Bill)** hỗ trợ chia đơn hàng cho nhóm khách thanh toán riêng lẻ.
     - Bấm nút **XÁC NHẬN THANH TOÁN**: Hệ thống cập nhật trạng thái hóa đơn thành `DaThanhToan`, giải phóng bàn ăn về trạng thái `Trong`, cộng dồn doanh thu vào ca làm việc hiện tại và hỗ trợ in hóa đơn nhiệt K80.
- **Hình ảnh kết quả demo**:

![Giao diện Màn hình Thu ngân & Thanh toán Hóa đơn](screenshots/ui_checkout.png)

---

#### 7.2.8. Phân hệ Quản lý Ca làm việc & Đối soát Két tiền mặt (Shift Management)
- **Mô tả kịch bản demo**:
  1. **Banner ca trực**: Hiển thị nhân viên trực ca, thời điểm mở ca, tiền mặt đầu ca, doanh thu tiền mặt, doanh thu chuyển khoản và **Tiền mặt lý thuyết trong két**.
  2. **Quy trình Mở ca (`OpenShiftDialog`)**: Thu ngân bắt đầu ca làm việc, hệ thống tự động gợi ý số tiền thực tế bàn giao từ ca liền trước để đối soát liên tục.
  3. **Quy trình Kết thúc ca & Bàn giao (`CloseShiftDialog`)**: Thu ngân kiểm đếm tiền mặt thực tế trong két và nhập vào hệ thống. Thuật toán tự động so khớp:
     - Tính `ChenhLech = TienThucTe - TienLyThuyet`.
     - Nếu phát sinh thừa hoặc thiếu tiền, hệ thống tô màu cảnh báo và bắt buộc thu ngân phải nhập giải trình lý do trước khi đóng ca.
     - Hỗ trợ in **Biên bản bàn giao ca làm việc** có chữ ký xác nhận của Thu ngân và Quản lý.
- **Hình ảnh kết quả demo**:

![Giao diện Quản lý Ca làm việc & Lịch sử ca](screenshots/ui_shift_management.png)

![Hộp thoại Mở ca làm việc (Open Shift Dialog)](screenshots/ui_open_shift_dialog.png)

![Hộp thoại Kết thúc ca & Đối soát két tiền mặt (Close Shift Dialog)](screenshots/ui_close_shift_dialog.png)

---

#### 7.2.9. Phân hệ Lịch sử Hóa đơn & Tra cứu Thanh toán (Invoice History)
- **Mô tả kịch bản demo**:
  1. Truy cập mục **Lịch sử hóa đơn** trên menu thanh toán.
  2. Hiển thị danh sách toàn bộ các hóa đơn đã lập trong quá khứ kèm ngày giờ, mã bàn, thu ngân lập hóa đơn, tổng tiền, hình thức thanh toán và mã ca trực tương ứng.
  3. Cho phép lọc theo khoảng ngày, tìm kiếm nhanh theo mã hóa đơn, nhấp chọn vào từng dòng để xem chi tiết danh mục món ăn đã phục vụ trong hóa đơn đó.
- **Hình ảnh kết quả demo**:

![Giao diện Phân hệ Tra cứu Lịch sử Hóa đơn](screenshots/ui_invoice_history.png)

---

#### 7.2.10. Các Phân hệ Quản trị Dữ liệu Danh mục (Master Data CRUD)
- **Mô tả kịch bản demo**:
  - Các màn hình quản trị được kế thừa đồng nhất từ lớp trừu tượng `BaseCrudPanel`:
    - **Tài khoản nhân viên (`UserManagementPanel`)**: Quản lý thông tin, phân quyền 5 vai trò (`Admin`, `QuanLy`, `ThuNgan`, `PhucVu`, `Bep`), trạng thái làm việc/nghỉ việc. Mật khẩu được mã hóa tự động bằng BCrypt và lưu vết thao tác vào Audit Log.
    - **Thực đơn món ăn (`MenuManagementPanel`)**: Quản lý danh sách món, đơn giá, danh mục trực thuộc, hình ảnh đại diện và trạng thái kinh doanh.
    - **Khuyến mãi & Voucher (`VoucherManagementPanel`)**: Quản lý mã giảm giá, tỷ lệ chiết khấu %, giảm tối đa, đơn hàng tối thiểu và ngày bắt đầu / kết thúc chiến dịch.
- **Hình ảnh kết quả demo**:

![Giao diện Quản trị Tài khoản Nhân viên & Phân quyền RBAC](screenshots/ui_users.png)

![Giao diện Quản trị Thực đơn Món ăn](screenshots/ui_menu_items.png)

![Giao diện Quản trị Khuyến mãi & Mã giảm giá Voucher](screenshots/ui_vouchers.png)

---

#### 7.2.11. Báo cáo Thống kê Doanh thu & Top Món ăn Bán chạy
- **Mô tả kịch bản demo**:
  - **Báo cáo Doanh thu (`RevenueReportPanel`)**: Tổng hợp doanh thu chi tiết theo ngày, hiển thị số lượng hóa đơn, doanh thu tiền mặt, doanh thu chuyển khoản và tổng doanh thu thực tế ghi nhận từ CSDL.
  - **Top Món Bán Chạy (`TopDishesReportPanel`)**: Xếp hạng các món ăn được thực khách ưa chuộng nhất dựa trên View `v_TopMonBanChay`, giúp ban quản lý đưa ra quyết định nhập hàng và xây dựng combo kích cầu hiệu quả.
- **Hình ảnh kết quả demo**:

![Giao diện Báo cáo Thống kê Doanh thu chi tiết](screenshots/ui_revenue_report.png)

![Giao diện Báo cáo Top Món ăn Bán chạy nhất](screenshots/ui_top_dishes.png)

---

### 7.3. Kế hoạch và Kết quả Kiểm thử Hệ thống (System Testing & Test Cases)

Kiểm thử phần mềm là giai đoạn then chốt trong chu trình phát triển hệ thống **RESTAURANT OPS**, nhằm đảm bảo toàn bộ các module nghiệp vụ, các thuật toán tính toán tài chính, kết nối cơ sở dữ liệu quan hệ MySQL và cơ chế phân quyền bảo mật vận hành chính xác 100% theo đúng đặc tả yêu cầu (SRS).

#### 7.3.1. Mục tiêu và Môi trường Kiểm thử (Testing Environment)
- **Mục tiêu kiểm thử**:
  1. *Kiểm thử Tính toàn vẹn CSDL (Database Integrity)*: Xác minh kết nối JDBC ổn định, schema 16 bảng và view đúng chuẩn, ràng buộc khóa ngoại (Foreign Keys) và không có lỗi suy biến dữ liệu.
  2. *Kiểm thử Logic Nghiệp vụ Cốt lõi (Core Business Logic)*: Tính toán đúng đắn tiền tạm tính, chiết khấu Voucher theo %, giảm tối đa, thuế VAT, đối soát két tiền ca làm việc tính chính xác `ChenhLech = TienThucTe - TienLyThuyet`.
  3. *Kiểm thử Xác thực & Bảo mật (Security & RBAC)*: Mật khẩu nhân viên được mã hóa một chiều chuẩn **BCrypt** (`$2a$10$...`), phân quyền 5 vai trò (`Admin`, `QuanLy`, `ThuNgan`, `PhucVu`, `Bep`) hiển thị đúng quyền hạn và tự động lưu vết an ninh vào **Nhật ký hệ thống (Audit Log)**.
  4. *Kiểm thử Hiệu năng Giao diện (GUI Performance)*: Cơ chế đa luồng bất đồng bộ **SwingWorker** không gây đơ cứng giao diện (non-blocking EDT) khi tải khối lượng dữ liệu lớn từ MySQL.
  5. *Kiểm thử Khả năng Chịu lỗi (Fault Tolerance)*: Bộ đệm ngoại tuyến `DummyDataFactory` tự động kích hoạt khi CSDL mất kết nối, đảm bảo nhà hàng không bị gián đoạn vận hành.
- **Môi trường thực thi kiểm thử**:
  - **Framework kiểm thử**: JUnit 5 Jupiter (`org.junit.jupiter:junit-jupiter:5.10.2`).
  - **Build Automation**: Apache Maven 3.8+ / Maven Surefire Plugin 3.5.4.
  - **Hệ điều hành**: Microsoft Windows 11 x64.
  - **Môi trường Java**: Java SE Development Kit (JDK 21 LTS).
  - **Hệ quản trị CSDL**: MySQL Server 8.0.46 Community.

#### 7.3.2. Bảng Danh mục Chi tiết các Kịch bản Kiểm thử (Test Cases Specification)

Hệ thống được kiểm thử tự động toàn diện qua **11 bài kiểm thử (Test Cases)** phân bổ trên 7 lớp kiểm thử chuyên biệt:

| Mã Test Case | Lớp Kiểm Thử (Test Class) | Phương Thức (Test Method) | Phân Hệ Nghiệp Vụ | Dữ Liệu & Thao Tác Kiểm Thử | Kết Quả Kỳ Vọng (Expected Result) | Đánh Giá |
|:---:|:---|:---|:---:|:---|:---|:---:|
| **TC01** | `DatabaseConnectionTest` | `testDatabaseConnectionAndTables` | CSDL & JDBC | Kết nối tới MySQL Server qua JDBC, đọc metadata danh sách bảng/views trong `restaurant_db`. | Kết nối thành công, nhận diện đủ 16 bảng & views nghiệp vụ. | **PASSED** ✅ |
| **TC02** | `DatabaseMigrationTest` | `migrateDatabase` | Di trú CSDL | Tự động tạo bảng `CaLamViec`, `LichSuTruyCap`, `LichSuThaoTac`, mở rộng ENUM `VaiTro` (`Admin`, `QuanLy`...), tạo Views. | Tạo mới / cập nhật schema thành công không phát sinh lỗi SQL syntax. | **PASSED** ✅ |
| **TC03** | `NhanVienDAOTest` | `testRoleAndStatusMapping` | Xác thực & RBAC | Kiểm tra hàm `toDbVaiTro`, `toDisplayVaiTro`, `toDbTrangThai`, `toDisplayTrangThai` với các biến thể chuỗi. | Chuyển đổi song ánh 100% giữa text tiếng Việt giao diện và mã ENUM DB. | **PASSED** ✅ |
| **TC04** | `NhanVienDAOTest` | `testInsertNhanVienWithVietnameseText` | Bảo mật & DAO | Chèn nhân viên mới với Họ tên tiếng Việt có dấu, mã hóa BCrypt, cập nhật thông tin và xóa dọn dẹp. | Thao tác thành công, không lỗi data truncation, mật khẩu được băm BCrypt. | **PASSED** ✅ |
| **TC05** | `MainFrameTest` | `testMainFrameInitializationAndCardSwitching` | Giao diện Swing | Khởi tạo cửa sổ `MainFrame`, kiểm tra tiêu đề, kích thước và điều hướng qua lại 12 thẻ `CardLayout`. | Mọi Panel module đều khởi tạo hợp lệ (not null), chuyển card mượt mà. | **PASSED** ✅ |
| **TC06** | `MainFrameTest` | `testRoleBasedAccessControl` | Phân quyền RBAC | Khởi tạo `MainFrame` với vai trò `Admin` và `QuanLy`, kiểm tra module mặc định và danh sách menu sidebar. | Admin mở `CARD_AUDIT_LOG`, Quản lý mở `CARD_DASHBOARD`, phân quyền chính xác. | **PASSED** ✅ |
| **TC07** | `MainFrameTest` | `testDummyDataOperations` | Bộ đệm Ngoại tuyến | Mở bàn mới, thêm món, thanh toán hóa đơn trên bộ đệm RAM `DummyDataFactory` khi ngắt MySQL. | Bàn chuyển trạng thái `Trong` $\leftrightarrow$ `CoKhach` chính xác không phát sinh lỗi. | **PASSED** ✅ |
| **TC08** | `KitchenAndShiftTest` | `testKitchenOperations` | Điều phối Bếp | Gọi món mới, Bếp trưởng bấm bắt đầu nấu (`DangLam`), hoàn thành (`DaRaMon`), hủy món và đổi tồn kho. | Chu trình trạng thái FSM chuyển đổi chính xác, cập nhật CSDL tức thì. | **PASSED** ✅ |
| **TC09** | `KitchenAndShiftTest` | `testShiftManagement` | Quản lý Ca | Mở ca làm việc với tiền đầu ca, truy vấn ca đang mở, đóng ca với tiền thực tế, tính chênh lệch thừa/thiếu. | Tiền lý thuyết và chênh lệch tính đúng công thức, cập nhật trạng thái `DaDong`. | **PASSED** ✅ |
| **TC10** | `ShiftPaymentTest` | `testShiftPaymentIntegration` | Tích hợp Nghiệp vụ | Chu trình khép kín: Mở bàn $\to$ Gọi món $\to$ Thu ngân Checkout $\to$ Giải phóng bàn $\to$ Cộng dồn ca trực. | Hóa đơn chuyển `DaThanhToan`, bàn về `Trong`, doanh thu ca tăng chính xác. | **PASSED** ✅ |
| **TC11** | `CaptureUiScreenshotsTest` | `captureScreenshots` | Tự động hóa UI | Khởi chạy ứng dụng thực tế, render giao diện và xuất file ảnh cho toàn bộ 17 màn hình và dialog. | Tạo đầy đủ 17 file ảnh `.png` chất lượng cao trong thư mục `screenshots/`. | **PASSED** ✅ |

#### 7.3.3. Đặc tả Kỹ thuật Chi tiết các Ca Kiểm thử Trọng yếu

##### 1. Kiểm thử Tích hợp Chu trình Thanh toán & Đối soát Doanh thu Ca (`ShiftPaymentTest`)
Kiểm tra tính nhất quán giữa hóa đơn thanh toán và tổng doanh thu ca làm việc của thu ngân:
```java
// Lấy ca làm việc đang mở ban đầu
CaLamViec caBanDau = caLamViecDAO.getCaDangMo();
double dtTienMatCu = caBanDau.getDoanhThuTienMat();

// Thực hiện thanh toán hóa đơn Bàn 9
boolean thanhToanOk = hoaDonDAO.thanhToanHoaDon(maHD, "TienMat", 0, 8);
assertTrue(thanhToanOk, "Thanh toán hóa đơn phải thành công");

// Lấy lại ca làm việc sau thanh toán và kiểm tra đối soát
CaLamViec caSau = caLamViecDAO.getCaDangMo();
assertEquals(dtTienMatCu + tongTienHD, caSau.getDoanhThuTienMat(), 0.01, 
    "Doanh thu tiền mặt ca phải được cộng dồn chính xác bằng tổng tiền hóa đơn");
assertEquals(caSau.getTienDauCa() + caSau.getDoanhThuTienMat(), caSau.getTienLyThuyet(), 0.01,
    "Tiền lý thuyết trong két phải bằng Tiền đầu ca + Doanh thu tiền mặt");
```

##### 2. Kiểm thử Bảo mật Mã hóa Mật khẩu BCrypt (`NhanVienDAOTest`)
Kiểm tra thuật toán băm một chiều an toàn chống lộ lọt thông tin nhân sự:
```java
String plainPassword = "admin123";
String hashedPassword = PasswordUtils.hashPassword(plainPassword);

// Kiểm tra định dạng chuỗi hash BCrypt
assertNotNull(hashedPassword);
assertTrue(hashedPassword.startsWith("$2a$10$"), "Chuỗi hash phải theo định dạng chuẩn BCrypt với cost factor 10");

// Kiểm tra tính đối soát mật khẩu
assertTrue(PasswordUtils.checkPassword(plainPassword, hashedPassword), "Đối soát mật khẩu đúng phải thành công");
assertFalse(PasswordUtils.checkPassword("saiMatKhau123", hashedPassword), "Đối soát mật khẩu sai phải trả về false");
```

##### 3. Kiểm thử Ma trận Phân quyền Vai trò RBAC (`MainFrameTest`)
Kiểm tra khả năng phân tách quyền hạn rạch ròi giữa Quản trị viên (Admin) và Quản lý (QuanLy):
```java
// 1. Kiểm tra tài khoản Quản trị viên (Admin)
NhanVien admin = new NhanVien(1, "Quản trị viên", "admin", "admin123", "Admin", "DangLam");
MainFrame adminFrame = new MainFrame(admin);
assertNotNull(adminFrame.getAuditLogPanel(), "Admin phải khởi tạo và truy cập được module Audit Log");
// Xác nhận màn hình mặc định của Admin là Nhật ký hệ thống (Audit Log)
assertEquals(MainFrame.CARD_AUDIT_LOG, adminFrame.getCurrentCardName());
adminFrame.dispose();

// 2. Kiểm tra tài khoản Quản lý (QuanLy)
NhanVien quanly = new NhanVien(2, "Quản lý Nhà hàng", "quanly", "quanly123", "QuanLy", "DangLam");
MainFrame quanLyFrame = new MainFrame(quanly);
assertNotNull(quanLyFrame.getDashboardPanel(), "Quản lý phải truy cập được màn hình Dashboard");
// Xác nhận màn hình mặc định của Quản lý là Tổng quan Dashboard
assertEquals(MainFrame.CARD_DASHBOARD, quanLyFrame.getCurrentCardName());
quanLyFrame.dispose();
```

#### 7.3.4. Nhật ký Thực thi Kiểm thử Tự động (Automated Test Execution Log - JUnit 5)

Dưới đây là trích xuất log thực tế khi chạy lệnh `mvn test` trên toàn bộ dự án:

```
[INFO] -------------------------------------------------------
[INFO]  T E S T S
[INFO] -------------------------------------------------------
[INFO] Running com.restaurant.CaptureUiScreenshotsTest
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 4.609 s -- in com.restaurant.CaptureUiScreenshotsTest
[INFO] Running com.restaurant.DatabaseConnectionTest
Kết nối thành công đến: MySQL 8.0.46
Các bảng hiện có trong restaurant_db:
 - ban
 - calamviec
 - chitiethoadon
 - danhmuc
 ...
 - v_topmonbanchay
 - voucher
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.034 s -- in com.restaurant.DatabaseConnectionTest
[INFO] Running com.restaurant.DatabaseMigrationTest
-> Bảng CaLamViec đã sẵn sàng!
-> Bảng HoaDon đã có cột MaCa.
-> View v_LichSuCaLamViec đã sẵn sàng!
-> Đã tạo tài khoản Admin mới: admin02 / admin123 (Role: Admin)
-> Đã phân tách vai trò Admin và QuanLy trong CSDL!
-> Toàn bộ bảng và view AuditLog (LichSuTruyCap, LichSuThaoTac) đã sẵn sàng!
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.227 s -- in com.restaurant.DatabaseMigrationTest
[INFO] Running com.restaurant.KitchenAndShiftTest
[INFO] Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.063 s -- in com.restaurant.KitchenAndShiftTest
[INFO] Running com.restaurant.MainFrameTest
[INFO] Tests run: 3, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 1.288 s -- in com.restaurant.MainFrameTest
[INFO] Running com.restaurant.NhanVienDAOTest
[INFO] Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.178 s -- in com.restaurant.NhanVienDAOTest
[INFO] Running com.restaurant.ShiftPaymentTest
-> Ca đang mở ban đầu: #2 | Tiền đầu ca: 1500000.0
-> Thực hiện thanh toán hóa đơn Bàn 9...
-> Doanh thu ca sau thanh toán: 918000.0 | Số HĐ: 2
-> Tiền mặt lý thuyết trong két: 2418000.0
-> Ca trước gần nhất: #1 | Nhân viên: Trần Thị Mai | Tiền thực tế bàn giao: 3450000.0
-> TOÀN BỘ QUY TRÌNH THANH TOÁN & ĐỒNG BỘ CA LÀM VIỆC ĐẠT 100% YÊU CẦU!
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.096 s -- in com.restaurant.ShiftPaymentTest
...
------------------------------------------------------------------------
Total time:  8.902 s
Finished at: 2026-09-23T13:15:30+07:00
------------------------------------------------------------------------
```

#### 7.3.5. Đánh giá Mức độ Tin cậy và Độ bao phủ (Reliability & Coverage Assessment)
1. **Tỷ lệ kiểm thử thành công**: Đạt **100% (11/11 tests PASSED)**, hoàn toàn không có lỗi ngoại lệ (`Exception`) hoặc thất bại xác nhận (`AssertionError`).
2. **Độ bao phủ đa tầng (Multi-layer Test Coverage)**:
   - *Tầng Cơ sở dữ liệu (Database Layer)*: Kiểm tra trọn vẹn kết nối JDBC, cấu trúc Schema, Triggers, Views và các ràng buộc toàn vẹn quan hệ.
   - *Tầng Truy cập Dữ liệu (DAO & Service Layer)*: Kiểm thử đầy đủ các thao tác CRUD, mã hóa bảo mật BCrypt, tính toán công thức doanh thu, đối soát ca làm việc và thuật toán FSM bếp.
   - *Tầng Trình diễn Giao diện (Presentation Layer)*: Kiểm thử thành công khả năng tương thích màn hình, chuyển thẻ `CardLayout`, sự kiện phân quyền động và render 17 hình ảnh UI chất lượng cao.
3. **Độ bền bỉ & Khả năng chịu tải (Robustness & Fault Tolerance)**: Hệ thống chứng minh khả năng duy trì hoạt động liên tục nhờ cơ chế `DummyDataFactory` tự động kích hoạt khi CSDL ngoại tuyến và cơ chế `SwingWorker` giữ cho giao diện luôn phản hồi mượt mà với người dùng.
