    ...
    public void captureScreenshots() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            try {
                ...
                // 13. Capture Audit Log
                frame.showCard(MainFrame.CARD_AUDIT_LOG);
                frame.validate();
                renderAndSave(frame.getContentPane(), new File(outputDir, "ui_audit_log.png"));

                // 14. Capture Login Dialog
                ...
                // 16. Capture Manager Frame (Full Access)
                MainFrame quanLyFrame = new MainFrame(new com.restaurant.model.NhanVien(2, "Trần Văn Quản Lý", "quanly", "quanly123", "QuanLy", "DangLam"));
                quanLyFrame.setSize(1366, 768);
                quanLyFrame.addNotify();
                quanLyFrame.validate();
                renderAndSave(quanLyFrame.getContentPane(), new File(outputDir, "ui_manager_dashboard.png"));
                quanLyFrame.dispose();
            } catch (Exception e) {
                e.printStackTrace();
            }
