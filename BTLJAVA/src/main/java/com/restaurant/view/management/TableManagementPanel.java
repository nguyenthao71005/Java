...
public class TableManagementPanel extends BaseCrudPanel {
    ...
    protected void onAdd() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiTaoMoi(null, "Ban",
                    String.format("Tạo bàn mới: %s (Khu vực: %s, Sức chứa: %d)", name, area, cap), null);
                loadData();
    ...
    protected void onEdit() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiCapNhat(null, "Ban",
                    String.format("Cập nhật bàn: %s", name),
                    String.format("Tên: %s, Khu vực: %s, Sức chứa: %s",
                    ...
                    String.format("Tên: %s, Khu vực: %s, Sức chứa: %d", name, area, cap));
                loadData();
    ...
    protected void onDelete() {
        ...
        if (opt == JOptionPane.YES_OPTION) {
            if (com.restaurant.database.DatabaseConnection.isConnected()) {
                ...
                if (ok) {
                    // Ghi log audit
                    auditService.ghiXoa(null, "Ban",
                        String.format("Xóa bàn: %s", name),
                        String.format("Mã: %d, Tên: %s, Khu vực: %s", 
                            maBan, name, tableModel.getValueAt(selected, 2).toString()));
                    loadData();
