...
/**
 * Lớp Base mở rộng từ BaseCrudPanel có hỗ trợ ghi nhận Audit Log tự động
 * Các CRUD Panel chỉ cần kế thừa class này và triển khai các method để tự động ghi log
 */
public abstract class BaseCrudPanelAudit extends JPanel {
    ...
    // Audit service
    protected AuditService auditService = AuditService.getInstance();
    
    // Tên bảng để ghi log (override trong constructor của lớp con)
    protected String tableName = "Unknown";

    public BaseCrudPanelAudit(String titleText, NhanVien currentUser) {
    ...
    /**
     * Override phương thức này để ghi log khi thêm mới
     * @return mô tả chi tiết thao tác
     */
    protected String getAuditMessageForAdd() {
        return "Thêm mới " + tableName;
    }
    
    /**
     * Override phương thức này để ghi log khi cập nhật
     * @return mô tả chi tiết thao tác
     */
    protected String getAuditMessageForEdit() {
        return "Cập nhật " + tableName;
    }
    
    /**
     * Override phương thức này để ghi log khi xóa
     * @return mô tả chi tiết thao tác
     */
    protected String getAuditMessageForDelete() {
        return "Xóa " + tableName;
    }
    
    /**
     * Override để trả về mô tả ngắn gọn của đối tượng (ví dụ: "Nhân viên: Nguyễn Văn A")
     * Dùng trong message ghi log
     */
    ...
    /**
     * Ghi log thêm mới - gọi sau khi thêm thành công
     */
    protected void logAddSuccess(String description) {
        auditService.ghiTaoMoi(currentUser, tableName, description, null);
    }
    
    /**
     * Ghi log cập nhật - gọi sau khi cập nhật thành công
     */
    protected void logEditSuccess(String description, String oldData, String newData) {
        auditService.ghiCapNhat(currentUser, tableName, description, oldData, newData);
    }
    
    /**
     * Ghi log xóa - gọi sau khi xóa thành công
     */
    protected void logDeleteSuccess(String description, String deletedData) {
        auditService.ghiXoa(currentUser, tableName, description, deletedData);
    }
