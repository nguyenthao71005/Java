...
public class MenuManagementPanel extends BaseCrudPanel {
    ...
    protected void onAdd() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiTaoMoi(null, "MonAn",
                    String.format("Tạo món mới: %s (Giá: %,.0f VND, Danh mục: %s)", 
                        name, price, dm != null ? dm.getTenDM() : "N/A"), null);
                loadData();
    ...
    protected void onEdit() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiCapNhat(null, "MonAn",
                    String.format("Cập nhật món: %s", name),
                    String.format("Tên: %s, Giá: %s, Danh mục: %s, Còn món: %s",
                        ...
                        tableModel.getValueAt(selected, 5).toString()),
                    String.format("Tên: %s, Giá: %s, Danh mục: %s, Còn món: %s",
                        name, UIConstants.formatCurrency(price), 
                        dm != null ? dm.getTenDM() : "N/A", conMon ? "Có" : "Không"));
                loadData();
    ...
    protected void onDelete() {
        ...
        if (opt == JOptionPane.YES_OPTION) {
            if (com.restaurant.database.DatabaseConnection.isConnected()) {
                ...
                // Ghi log audit
                auditService.ghiXoa(null, "MonAn",
                    String.format("Xóa món: %s (Mã: %d)", name, maMon),
                    String.format("Mã: %d, Tên: %s, Giá: %s", 
                        maMon, name, tableModel.getValueAt(selected, 3).toString()));
                loadData();
