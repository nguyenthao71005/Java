...
public class VoucherManagementPanel extends BaseCrudPanel {
    ...
    protected void onAdd() {
        ...
        if (DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiTaoMoi(null, "Voucher",
                    String.format("Tạo voucher: %s (Giảm %s%%, Tối đa %s)", 
                        code, percent, UIConstants.formatCurrency(maxDisc)), null);
                loadData();
    ...
    protected void onEdit() {
        ...
        if (DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiCapNhat(null, "Voucher",
                    String.format("Cập nhật voucher: %s", code),
                    String.format("Mã: %s, Tên: %s, Giảm: %s%%, Trạng thái: %s",
                        ...
                        tableModel.getValueAt(selected, 6).toString()),
                    String.format("Mã: %s, Tên: %s, Giảm: %s%%, Trạng thái: %s",
    ...
    protected void onDelete() {
        ...
        if (opt == JOptionPane.YES_OPTION) {
            if (DatabaseConnection.isConnected()) {
                // Ghi log audit trước khi xóa
                auditService.ghiXoa(null, "Voucher",
                    String.format("Xóa voucher: %s", code),
                    String.format("Mã: %s, Tên: %s", code, tableModel.getValueAt(selected, 1).toString()));
