...
public class CategoryManagementPanel extends BaseCrudPanel {
    ...
    protected void onAdd() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiTaoMoi(null, "DanhMuc",
                    String.format("Tạo danh mục: %s", name), null);
                loadData();
    ...
    protected void onEdit() {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            ...
            if (ok) {
                // Ghi log audit
                auditService.ghiCapNhat(null, "DanhMuc",
                    String.format("Cập nhật danh mục: %s", name),
                    String.format("Tên: %s, Mô tả: %s",
                        tableModel.getValueAt(selected, 1).toString(),
                        tableModel.getValueAt(selected, 2).toString()),
                    String.format("Tên: %s, Mô tả: %s", name, desc));
                loadData();
    ...
    protected void onDelete() {
        ...
        if (opt == JOptionPane.YES_OPTION) {
            if (com.restaurant.database.DatabaseConnection.isConnected()) {
                ...
                if (ok) {
                    // Ghi log audit
                    auditService.ghiXoa(null, "DanhMuc",
                        String.format("Xóa danh mục: %s", name),
                        String.format("Mã: %d, Tên: %s", maDM, name));
                    loadData();
