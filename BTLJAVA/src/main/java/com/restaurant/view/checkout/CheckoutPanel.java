...
public class CheckoutPanel extends JPanel {
    ...
    public void refreshOccupiedTables() {
        ...
        List<Ban> occupied = new ArrayList<>();
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            for (Ban b : new com.restaurant.dao.BanDAO().getAll()) {
                if (b.isCoKhach()) occupied.add(b);
            }
        } else {
            occupied = DummyDataFactory.getInstance().getDanhSachBanCoKhach();
        }
    ...
    private void loadBillingForTable(Ban ban) {
        ...
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            currentHoaDon = new com.restaurant.dao.HoaDonDAO().getActiveInvoiceByTable(ban.getMaBan());
        } else {
    ...
    private void handleCheckoutPayment() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (com.restaurant.database.DatabaseConnection.isConnected() && currentHoaDon != null) {
                new com.restaurant.dao.HoaDonDAO().checkout(currentSelectedBan.getMaBan(), currentHoaDon.getMaHD(), method, discountRate, vatRate);
            }
            DummyDataFactory.getInstance().thanhToanBan(currentSelectedBan.getMaBan(), method, discountRate, vatRate);

            JOptionPane.showMessageDialog(this,
                    "Thanh toán thành công!\n" +
                            "Bàn [" + currentSelectedBan.getTenBan() + "] đã được thanh toán hoàn tất và chuyển về trạng thái 'Trống' (Xanh lá).",
                    "Thanh toán hoàn tất",
                    JOptionPane.INFORMATION_MESSAGE);
