...
public class MainFrame extends JFrame {
    ...
    public static final String CARD_AUDIT_LOG = "AUDIT_LOG";
    ...
    public MainFrame() {
        this(new NhanVien(1, "Quản trị viên", "admin", "admin123", "Admin", "DangLam"));
    }

    public MainFrame(NhanVien user) {
        this.currentUser = (user != null) ? user : new NhanVien(1, "Quản trị viên", "admin", "admin123", "Admin", "DangLam");
    ...
    private void initLayout() {
        ...
        // Mặc định hiển thị theo phân quyền RBAC
        String role = com.restaurant.dao.NhanVienDAO.toDbVaiTro(currentUser.getVaiTro());
        boolean isAdmin = "Admin".equalsIgnoreCase(role);
        boolean isQuanLy = "QuanLy".equalsIgnoreCase(role);

        if (isAdmin) {
            showCard(CARD_AUDIT_LOG);
        } else if (isQuanLy) {
            showCard(CARD_DASHBOARD);
        } else if ("Bep".equalsIgnoreCase(role)) {
            showCard(CARD_KITCHEN);
        } else {
            showCard(CARD_TABLE_MAP);
        }
    }
    ...
    private void initModules() {
        ...
        // Module: Audit Log (Nhật ký hệ thống)
        auditLogPanel = new AuditLogPanel();
        centerContentPanel.add(auditLogPanel, CARD_AUDIT_LOG);
    }
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblUser = new JLabel("Đăng nhập: " + currentUser.getHoTen() + " (" + com.restaurant.dao.NhanVienDAO.toDisplayVaiTro(currentUser.getVaiTro()) + ") ");
    ...
    private JPanel createSidebarPanel() {
        ...
        boolean isAdmin = "Admin".equalsIgnoreCase(role);
        boolean isQuanLy = "QuanLy".equalsIgnoreCase(role);
        boolean isCashier = "ThuNgan".equalsIgnoreCase(role);
        boolean isWaiter = "PhucVu".equalsIgnoreCase(role);
        boolean isChef = "Bep".equalsIgnoreCase(role);

        // 1. Nhóm HỆ THỐNG (Admin & Quản lý)
        if (isAdmin || isQuanLy) {
            menuContainer.add(createSidebarHeader("HỆ THỐNG"));
            if (isAdmin) {
                // Admin: ưu tiên xem Audit Log trước, sau đó là Dashboard hệ thống
                menuContainer.add(createSidebarItem("Nhật ký hệ thống", "\uD83D\uDCDC", CARD_AUDIT_LOG));
        ...
        // 2. Nhóm DỊCH VỤ & BÀN ĂN (Quản lý, Phục vụ, Thu ngân, Bếp) - ADMIN KHÔNG THẤY
        if (!isAdmin) {
            menuContainer.add(createSidebarHeader("DỊCH VỤ & BÀN ĂN"));
            if (isQuanLy || isCashier || isWaiter) {
        ...
        // 3. Nhóm THU NGÂN & HÓA ĐƠN (Quản lý, Thu ngân) - ADMIN KHÔNG THẤY
        if (isQuanLy || isCashier) {
        ...
        // 4. Nhóm QUẢN TRỊ DỮ LIỆU (Admin & Quản lý)
        if (isAdmin || isQuanLy) {
        ...
        // 5. Nhóm BÁO CÁO THỐNG KÊ (Chỉ Quản lý - Toàn quyền cả hệ thống; Admin không phụ trách kinh doanh)
        if (isQuanLy) {
        ...
        JPanel itemAccount = createSidebarActionItem("Thông tin tài khoản", "\uD83D\uDC64", () -> {
            JOptionPane.showMessageDialog(this,
                    ...
                    "Vai trò: " + com.restaurant.dao.NhanVienDAO.toDisplayVaiTro(currentUser.getVaiTro()) + "\n" +
                    "Trạng thái: " + com.restaurant.dao.NhanVienDAO.toDisplayTrangThai(currentUser.getTrangThai()) + "\n" +
        ...
        JPanel itemLogout = createSidebarActionItem("Đăng xuất hệ thống", "\u27A1", () -> {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                // Ghi nhận đăng xuất vào Audit Log
                AuditService.getInstance().ghiDangXuat(currentUser);
                dispose();
    ...
    public void showCard(String cardName) {
        ...
        } else if (CARD_AUDIT_LOG.equals(cardName) && auditLogPanel != null) {
    ...
    public AuditLogPanel getAuditLogPanel() {
