...
public class TableMapPanel extends JPanel {
    ...
    public void refreshTableGrid(String floorFilter) {
        ...
        if (DatabaseConnection.isConnected()) {
            allTables = banDAO.getAll();
        } else {
            allTables = DummyDataFactory.getInstance().getDanhSachBan();
        }
    ...
    private void handleTableClick(Ban ban) {
        if (!ban.isCoKhach()) {
            ...
            if (opt == JOptionPane.YES_OPTION) {
                // Chuyển trạng thái bàn sang "CoKhach" và tạo hóa đơn trong CSDL MySQL
                if (DatabaseConnection.isConnected()) {
                    hoaDonDAO.createInvoiceForTable(ban.getMaBan(), ban.getTenBan());
                    banDAO.updateStatus(ban.getMaBan(), "CoKhach");
                    ban.setTrangThai("CoKhach");
                }
