...
public class OrderPanel extends JPanel {
    ...
    private JPanel createMenuSelectionPanel() {
        ...
        List<DanhMuc> categories = com.restaurant.database.DatabaseConnection.isConnected()
                ? new com.restaurant.dao.DanhMucDAO().getAll()
    ...
    public void loadOrderForTable(int maBan) {
        ...
        Ban b = null;
        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            for (Ban item : new com.restaurant.dao.BanDAO().getAll()) {
        ...
        if (b == null) {
            b = DummyDataFactory.getInstance().timBanTheoMa(maBan);
        }
        String tenBan = (b != null) ? b.getTenBan() : ("Bàn " + maBan);
        lblCurrentTable.setText("Đơn hàng hiện tại: " + tenBan + " ");

        if (com.restaurant.database.DatabaseConnection.isConnected()) {
            currentHoaDon = new com.restaurant.dao.HoaDonDAO().getActiveInvoiceByTable(maBan);
            if (currentHoaDon == null) {
                currentHoaDon = new com.restaurant.dao.HoaDonDAO().createInvoiceForTable(maBan, tenBan);
            }
        } else {
        ...
        currentItemsList = (currentHoaDon != null && currentHoaDon.getDanhSachChiTiet() != null)
    ...
    private void addDishToOrder(MonAn mon) {
        ...
        ChiTietHoaDon newItem = new ChiTietHoaDon(
                ...
                currentHoaDon != null ? currentHoaDon.getMaHD() : 0,
                mon.getMaMon(),
                mon.getTenMon(),
                1,
                mon.getGiaBan(),
    ...
    private void syncOrderItemsToDatabase() {
    ...
    private void handleTransferTable() {
        List<Ban> allTables = com.restaurant.database.DatabaseConnection.isConnected()
                ? new com.restaurant.dao.BanDAO().getAll()
                : DummyDataFactory.getInstance().getDanhSachBan();
        List<Ban> emptyTables = new ArrayList<>();
        ...
        if (target != null) {
            Ban targetBan = null;
            for (Ban b : emptyTables) {
                if (b.getTenBan().equals(target)) {
                    targetBan = b;
                    break;
                }
            }

            if (com.restaurant.database.DatabaseConnection.isConnected() && targetBan != null) {
                boolean ok = new com.restaurant.dao.HoaDonDAO().transferTable(currentMaBan, targetBan.getMaBan());
                if (ok) {
                    currentMaBan = targetBan.getMaBan();
                    loadOrderForTable(currentMaBan);
                    if (mainFrame != null && mainFrame.getTableMapPanel() != null) {
                        mainFrame.getTableMapPanel().refreshTableGrid("Tất cả");
                    }
                    JOptionPane.showMessageDialog(this,
                            "Đã chuyển toàn bộ hóa đơn từ bàn cũ sang " + target + " thành công trong CSDL MySQL!",
